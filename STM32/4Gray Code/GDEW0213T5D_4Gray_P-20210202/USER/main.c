...
unsigned char BlackImage[2756];//Define canvas space  
...
int	main(void)
{
  ...
  //4GRAY
    EPD_init_4GRAY(); //EPD init 4GRAY
	  PIC_display_4GRAY(gImage_4GRAY);//EPD_picture
		EPD_sleep();//EPD_sleep,Sleep instruction is necessary, please do not delete!!!
		delay_s(2); //2s	
   ...
   //PARTIAL REFRESH
		...
		EPD_Dis_Part(0,0,gImage_2,212,104,0); //Refresh background color			
		EPD_Dis_Part(0,0,gImage_num1,32,32,0); //partial display 1    //x_start,y_start,picData,column,line
		EPD_Dis_Part(0,0,gImage_num2,32,32,1); //partial display 2
		EPD_Dis_Part(0,0,gImage_num3,32,32,1); //partial display 3


		/*Partial refresh 5 times (maximum), perform a full refresh operation.
			Otherwise, the afterimage may not be eliminated	*/		
		//Clear
		EPD_init(); //EPD init
	  PIC_display_Clear();//EPD Clear
		EPD_sleep();//EPD_sleep,Sleep instruction is necessary, please do not delete!!!
