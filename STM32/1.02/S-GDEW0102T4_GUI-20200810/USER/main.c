  ...
  ///////////////////////////GUI///////////////////////////////////////////////////////////////////////////////////
   //Data initialization settings
    ...
    Paint_DrawPoint(5, 55, BLACK, DOT_PIXEL_4X4, DOT_STYLE_DFT);
		//Line
    Paint_DrawLine(20, 5, 50, 35, BLACK, LINE_STYLE_SOLID, DOT_PIXEL_1X1);
    Paint_DrawLine(50, 5, 20, 35, BLACK, LINE_STYLE_SOLID, DOT_PIXEL_1X1);
		//Rectangle
    Paint_DrawRectangle(20, 5, 50, 35, BLACK, DRAW_FILL_EMPTY, DOT_PIXEL_1X1);
    Paint_DrawRectangle(70, 5, 100, 35, BLACK, DRAW_FILL_FULL, DOT_PIXEL_1X1);
    //Circle
		Paint_DrawCircle(30, 50, 10, BLACK, DRAW_FILL_EMPTY, DOT_PIXEL_1X1);
    Paint_DrawCircle(80, 50, 10, BLACK, DRAW_FILL_FULL, DOT_PIXEL_1X1);
    ...
    /***********String***************************/ 
		EPD_init(); //EPD init
		Paint_Clear(WHITE);
    Paint_DrawString_EN(0, 0, "Hello", &Font8, WHITE, BLACK);  //5*8
		Paint_DrawString_EN(0, 8, "Hello", &Font12, WHITE, BLACK); //7*12
		Paint_DrawString_EN(0, 20, "Hello", &Font16, WHITE, BLACK); //11*16
		Paint_DrawString_EN(0, 36, "Hello", &Font20, WHITE, BLACK); //14*20
		Paint_DrawString_EN(0, 56, "Hello", &Font24, WHITE, BLACK); //17*24
    EPD_Display(BlackImage); //display image
		EPD_sleep();//EPD_sleep,Sleep instruction is necessary, please do not delete!!!
    delay_s(2); //2s		
		/*************Number************************/
		EPD_init(); //EPD init
		Paint_Clear(WHITE);
    Paint_DrawNum(0, 0, 1234567, &Font8, WHITE, BLACK);  //5*8
		Paint_DrawNum(0, 8, 1234567, &Font12, WHITE, BLACK); //7*12
		Paint_DrawNum(0, 20, 1234567, &Font16, WHITE, BLACK); //11*16
		Paint_DrawNum(0, 36, 1234567, &Font20, WHITE, BLACK); //14*20
		Paint_DrawNum(0, 56, 1234567, &Font24, WHITE, BLACK); //17*24
    EPD_Display(BlackImage); //display image
		EPD_sleep();//EPD_sleep,Sleep instruction is necessary, please do not delete!!!
    delay_s(2); //2s	 			
