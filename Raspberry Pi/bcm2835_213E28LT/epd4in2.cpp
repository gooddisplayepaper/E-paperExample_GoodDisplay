...
int Epd::Init(void) {
    ...
    SendData(0x00);
    SendData(0x0C);    //0x0C-->(12+1)*8=104

    SendCommand(0x45); //set Ram-Y address start/end position
    SendData(0xD3);   //0xD3-->(211+1)=212
    SendData(0x00);
    SendData(0x00);
    SendData(0x00);
...
void Epd::DisplayFrame(const unsigned char* frame_buffer) {
    if (frame_buffer != NULL) {
          ...
          WaitUntilIdle();
           ...
           {
             tempOriginal=*(frame_buffer+templine*MAX_COLUMN_BYTES+tempcol); //x*104+y
             templine++;
...
void Epd::DisplayFrame1(const unsigned char* frame_buffer) {
    if (frame_buffer != NULL) {
          ...
          WaitUntilIdle();
            ...
            //if(RED)
            //Epaper_Write_Command(0x26);   //write RAM for black(0)/white (1)
            for(i=0;i<ALLSCREEN_BYTES;i++)  //2756
           {
             tempOriginal=*(frame_buffer+ALLSCREEN_BYTES-(MAX_LINE_BYTES*tempcol-i%MAX_LINE_BYTES)); //(2756-(13*y-i%13))
             templine++;
             if(templine>=MAX_LINE_BYTES)  //x>=104
             {
               tempcol++;
               templine=0;
             }
